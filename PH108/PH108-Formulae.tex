...
\begin{figure}[h]
	\centering
	\includegraphics{D:/IIT RESOURCES/PH 108/operators_spherical.png}
\end{figure}\\
Cylindrical:\\
\begin{figure}[h]
	\centering
	\includegraphics{D:/IIT RESOURCES/PH 108/operators_cyl.png}
\end{figure}
...
\begin{equation}
	V(\va{r})=\frac{1}{4\pi \epsilon_0}\int \frac{\rho(\va{r'})}{\abs{\va{r}-\va{r'}}}d\tau'
\end{equation}
Poisson's equation: 
\begin{equation}
...
\begin{tcolorbox}
	\begin{itemize}
		\item Gauss's Law: $$\oint \va{E}\cdot d\va{a}=\frac{Q_{en}}{\epsilon_0}$$
...
\subsection{Separations of Variables(Cartesian)}
...
\begin{enumerate}[label=\roman*]
    ...
    \item  	$\sinh(x)=\dfrac{e^{kx}-e^{-kx}}{2}$
    \item   $\cosh(x)=\dfrac{e^{kx}+e^{-kx}}{2}$
    \end{enumerate}
\end{enumerate}
...
\subsection{Separation of Variables(Spherical Polar)}
...
The n=0 term is the potential due to the monopole a which basically considering a point charge equal to the total charge of the configuration placed at origin and it's potential calculated the normal way at $\va{r}$.
\begin{equation}
